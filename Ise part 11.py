...
def get_key_signature():
    ...
    print("Examples: G major, D major, E minor, Bb major, F# minor")

    key_input = input("Key: ").strip()
    parts = key_input.split()

    if len(parts) != 2:
        print("Invalid format. Use 'D major' or 'E minor'.")
        return get_key_signature()

    tonic = parts[0]
    mode = parts[1].lower()

    # Convert flats to music21 format
    tonic = tonic.replace("Bb", "B-").replace("Eb", "E-").replace("Ab", "A-").replace("Db", "D-").replace("Gb", "G-")

    if mode not in ["major", "minor"]:
        print("Mode must be 'major' or 'minor'.")
        return get_key_signature()
...
def create_measure_from_input(measure_number):
    ...
    print("  note C4 0.25 (Semiquaver)")
    print("  note C4 0.5 (Quaver")
    print("  note F#4 1 (Crotchet)")
    print("  note F#4 1.5 (Dotted crotchets)")
    print("  note F#4 2 (Minim)")
    print("  note C4 3 (Dotted minim)")
    print("  note C4 4 (Semi-breve)")
    print("  rest 0.5 (These are just examples, they can be any note you like)")
