...
s = stream.Score()#This now acts as a list, this is the score that we will add all the notes and instruments to

s.metadata = metadata.Metadata()
s.metadata.title = "Jesu, Joy of Man's Desiring"
s.metadata.composer = "J.S Bach"
'''
tempo_mark = tempo.MetronomeMark(number = 68, referent = note.Note(type="quarter", dots = 1))
s.insert(0, tempo_mark) # This didn't work
'''
ts = meter.TimeSignature("9/8") # This is the time signature that the piece requires
ks = key.KeySignature(1) #This is the key of G major
...
#The parts now have their proper names
...
violin3.append(ts)
cello.append(ts)
#This adds the time signature to the different parts
violin1.append(ks)
violin2.append(ks)
violin3.append(ks)
cello.append(ks)
#This adds the key signature to the different parts
